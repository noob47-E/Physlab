...
import { betweenArea, slopeAt } from '../math/graphs'
 ...
 */
export function visualizeTangentAt(expr: string, a: number): { fa: number; slope: number } {
  ...
  visualizeTangent(expr, a, fa, slope)
  return { fa, slope }
}
 ...
 * Returns the area so the caller can say it in words too.
 */
export function visualizeBetween(upper: string, lower: string, a: number, bnd: number, label?: string): number {
