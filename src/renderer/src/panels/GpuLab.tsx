  ...
  return (
    <div className="panel p-3">
      ...
      <div className="mb-3 text-[12px] text-zinc-500">
        Renderer: <span className="text-zinc-300">{gpu.backend}</span> {gpu.adapter && `· ${gpu.adapter}`}
        {gpu.backend !== 'WebGPU' && <span className="ml-2 text-amber-300">(GPU compute needs WebGPU)</span>}
      </div>
      <div className="grid max-w-3xl grid-cols-[110px_1fr] items-center gap-x-3 gap-y-2">
        <label className="text-zinc-400">Enabled</label>
        <div>
        ...
        <label className="text-zinc-400">Particles</label>
        ...
        <label className="text-zinc-400">B field (x, y, z)</label>
